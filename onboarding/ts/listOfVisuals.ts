...
function createToogleVisabilityButton(visualID){
    let visualData = helpers.getDataWithId(visualID);
...
export function toggleVisability(listElement){
    let disableElement = listElement.querySelector(".disableVisualButton");
    let visualData = helpers.getDataWithId(listElement.id);
...
async function createCollapseForm(visualID){
    let visualData = helpers.getDataWithId(visualID);
    let visualInfos = await getVisualInfos(visualData);
...
async function getVisualInfos(visualData){
    ...
    return {generalInfos, dataInfos, interactionInfos};
}

export function addVisualTextarea(category, visualID){
