...
async function resetVisualsInfo(){
    ...
    global.allVisuals.forEach(async function (visual: { title: any; }) {
...
export function orderVisuals(){
    var newOrderVisuals: any[] = [];
    let draggables = document.getElementsByClassName("draggable");
    [].forEach.call(draggables, function (draggable: any) {
        let visual = global.allVisuals.find(function (visual: any) {
            return visual.name == draggable.id;
        });
        let visualData = global.settings.visuals.find(function (visualData: global.SettingsVisual) {
