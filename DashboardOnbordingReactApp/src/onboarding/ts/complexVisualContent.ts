...
export async function getLineClusteredColumnComboChartInfo(visual: any) {
    ...
    let generalImages = [] as any[];
    let generalInfos = [] as string[];
    let dataImages = [] as any[];
    let dataInfos = [] as string[];
    let interactionImages = [] as any[];
    let interactionInfos = [] as string[];
...
export async function getLineClusteredColumnComboChartChanges(visual: any) {
    ...
    let additionalFilters = global.selectedTargets.filter(function (data: { target: { column: string; }; }) {
