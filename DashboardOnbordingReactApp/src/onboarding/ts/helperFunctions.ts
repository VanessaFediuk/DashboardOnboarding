...
import { IBasicFilter, IFilterColumnTarget, IFilterMeasureTarget } from "powerbi-models";
import 'powerbi-report-authoring';
import { VisualDescriptor } from "powerbi-client";

export function addContainerOffset(){
    var headerHeight = document.getElementById("header")!.clientHeight;
    var topOffset = global.interactionCardHeight - headerHeight;

    var container = document.getElementById("embed-container");
    if(container){
        var containerOffset = topOffset + 10;
    ...
    if(onboarding){
        ...
        var top =  global.onboardingOffset + topOffset;
...
export async function getActivePage(){
    ...
    let page = pages.filter(function(page: { isActive: any; }) {
...
export async function getFieldColumn(visual: VisualDescriptor, fieldName: string){
    let column: string = "";
    let fields = await visual.getDataFields(fieldName) as IFilterColumnTarget[];
    if(fields.length != 0){
...
export async function getFieldMeasure(visual: VisualDescriptor, fieldName: string){
    let measure: string = "";
    let fields = await visual.getDataFields(fieldName) as IFilterMeasureTarget[];
    if(fields.length != 0){
...
export async function getFilterInfo(){
    ...
    for (let i = 0; i < filters.length; ++i) {
        let filter = filters[i] as IBasicFilter;
...
export async function getVisualData(visual: any){
    ...
    for (let i = 0; i < headers.length; i++) {
        ...
        if(!isNaN(Number(dataArray[0]))){
            dataArray = dataArray.map((str: string) => {
                return Number(str);
            });
...
export async function getVisualInfos(visual: any){
    let type = getTypeName(visual);
    var visualInfos = {generalImages:[] as any[],generalInfos:[] as string[],dataImages:[] as any[],dataInfos:[] as string[],interactionImages:[] as any[],interactionInfos:[] as string[]};
