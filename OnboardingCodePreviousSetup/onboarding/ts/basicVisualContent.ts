...
export async function getCardInfo(visual) {
...
export async function getCardChanges(visual) {
    let dataName = await helpers.getFieldMeasure(visual, "Values");
    let dataValue = await helpers.getSpecificDataInfo(visual, dataName);
...
export async function getSlicerInfo(visual) {
...
export async function getSlicerInteractionExample(visual) {
    let filterName = await helpers.getFieldColumn(visual, "Values");
...
export async function getChartChanges(visual) {
    ...
    let additionalFilters = global.selectedTargets.filter(function (selectedData) {
